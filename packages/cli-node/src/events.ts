...
// everything mcplock can report to a developer, so the CLI never surfaces
...
const RULES: ClassifierRule[] = [
  ...
  },
  {
    ...
    describe: d({
      ...
      remediation: ["Retry — a slow server start (e.g. npx cold cache) can cause a one-off timeout.", "mcplock doctor"],
      retryable: true,
      requiresNetwork: false,
      requiresApproval: false,
    }),
