...
Current step: item 3 (Postgres migration tooling) — schema+migrations done for BOTH services/app-api and services/ingest, verified against two separate real Docker postgres:16-alpine containers (cleaned up afterward). Deliberately NOT doing the db.ts/app.ts runtime cutover autonomously — see NIGHT_SHIFT_LOG.md for why (security-sensitive RBAC/auth data layer, wants a human checkpoint per CLAUDE.md).
Last verified working: app-api's 12-table schema + ingest's 7-table schema (with the one real intentional difference — ingest's events.prev_hash/chain_hash/batch_signature are NOT NULL, matching its stricter SQLite constraint, plus the idx_events_workspace_ts index app-api's copy doesn't have) both applied cleanly to real Postgres containers with psql-verified shapes. Full TS regression (205 tests) still green throughout (both schema.ts files are additive/unused by the running app).
Blockers: none — next candidates: (a) the reviewed db.ts/app.ts cutover itself (needs explicit human checkpoint per CLAUDE.md's security-sensitive-code rule, not to be done autonomously), (b) Python CLI parity (Morning Action Item #2, largest remaining gap)
