...
```
Current milestone: 2 — MILESTONE 2 COMPLETE, all 7 steps done (npx side; uvx/cli-python CLI explicitly deferred, see Change Log). Awaiting user confirmation before starting Milestone 3 (per Tasks.md session protocol item 5).
...
## Milestone 3 — Ingest + Event Store *(stub — expand when we get here)*

Reference: `build-bible.md` Part 4, Part 5.2. High-level steps to expand into full detail at the time: stand up Postgres + event store, implement the Ingest API's signature verification (Part 4.3), implement `mcplock login`'s device-authorization flow and OS-keychain storage, wire opt-in event shipping from the local log (2.5) to the Ingest API.
