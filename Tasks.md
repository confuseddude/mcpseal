...
```
Current milestone: post-6 hardening — all 6 milestones complete (see below); working through NIGHT_SHIFT_LOG.md's "Morning Action Items" gap list one item at a time under continued autonomous-execution authorization. Item 4 (Dashboard-authenticated device-flow approval) done 2026-08-19: `POST /v1/machines/connect` on services/app-api (session-auth, admin+ RBAC, cross-org-safe workspaceId resolution) directly approves a pending device_codes row in the shared physical DB file — no HTTP call to services/ingest, same "shared file, no cross-service import" pattern as workspaces/machines/api_keys. Dashboard Settings page has a real "Connect a machine" form wired to it. `mcplock login` now works end-to-end without MCPLOCK_DEV_AUTO_APPROVE_DEVICE or a raw curl call. See NIGHT_SHIFT_LOG.md's new section for detail/tests/real e2e run.
Current step: item 3 (Postgres migration tooling) started, scoped to schema+migrations for app-api only — see NIGHT_SHIFT_LOG.md's "partial" section for why the runtime db.ts cutover is deliberately deferred to a reviewed follow-up rather than done unilaterally
Last verified working: real drizzle-kit generate -> real Docker postgres:16-alpine -> real db:migrate -> psql-verified table/column shapes match the SQLite dev schema exactly -> real drizzle-orm insert/select round-trip. Container cleaned up afterward. Full TS regression (205 tests) still green (schema.ts is additive, unused by the running app). services/ingest has no Postgres schema staged yet.
Blockers: none — next candidates: (a) services/ingest's equivalent schema/migrations, (b) the reviewed db.ts/app.ts cutover itself (needs explicit human checkpoint per CLAUDE.md's security-sensitive-code rule, not to be done autonomously), (c) Python CLI parity (Morning Action Item #2, largest remaining gap)
