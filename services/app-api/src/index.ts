...
const dbPath = process.env.MCPSEAL_CONTROL_PLANE_DB ?? process.env.MCPSEAL_INGEST_DB ?? path.join(homedir(), ".mcpseal", "ingest-dev.sqlite3");
...
app
  ...
  .then(() => {
    console.log(`mcpseal app-api (dev) listening on http://127.0.0.1:${port}`);
  })
  .catch((err) => {
    console.error(err);
    process.exit(1);
